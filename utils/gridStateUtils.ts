...
export const initGrid = (): GridElementState[] => {
    let grid: GridElementState[] = [];

    for (let y = 0; y < N; y++) {
        for (let x = 0; x < N; x++) {
            let element: GridElementState = emptyElement(x, y);
            grid.push(element)
        }
    }
...
export const hasPair = (grid: GridElementState[], e: GridElementState): boolean => {
    return getPairs(grid, e).length > 0;
}
...
export const getPairs = (grid: GridElementState[], e: GridElementState): GridElementState[] => {
    return grid.filter(each => isPairTop(each, e) || isPairBottom(each, e) || isPairLeft(each, e) || isPairRight(each, e));
}
...
export const getFinalJokerElement = (grid: GridElementState[], e: GridElementState): GridElementState => {
    let sortedAdjacentElements: GridElementState[] = getAdjacent(grid, e)
        .filter(each => each.type !== GridElementType.EMPTY)
        .sort((v1, v2) => compareTwoElements(v1, v2));
