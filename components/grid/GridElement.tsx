...
export const GridElement = ({
  ...
  const handlePress = (element: GridElementState): void => {
    ...
    const elementToUpdate = newGrid.find((each) =>
      each.x === element.x && each.y === element.y);
    if (!elementToUpdate) {
      return;
    }

    if (newElement.type === GridElementType.JOKER) {
      const finalJokerElement = getFinalJokerElement(newGrid, elementToUpdate);
    ...
    while (hasPair(newGrid, elementToUpdate)) {
      noOfComboMerges += 1;
      const pairElements: GridElementState[] = getPairs(newGrid, elementToUpdate);

      if (elementToUpdate.type === GridElementType.VALUE) {
        const bonusPoints = (pairElements.length - 1) * elementToUpdate.value;
        elementToUpdate.bonus = bonusPoints > 0;
        points += bonusPoints;
      }
      elementToUpdate.value *= 2;
  ...
  const getLevelElement = (x: number, y: number) => {
    const levelElements = levelGrid.filter((l) =>
      l.x === x && l.y === y);
