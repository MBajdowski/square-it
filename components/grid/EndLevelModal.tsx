...
const isElementTheSame = (element1: GridElementState, element2: GridElementState) => {
    return element1.x === element2.x && element1.y === element2.y && element1.type === element2.type && element1.value === element2.value;
};
...
const isSubset = (superset: GridElementState[], subset: GridElementState[]) => {
    ...
    subset.forEach(element => {
        if (superset.filter(each => isElementTheSame(each, element)).length === 0) {
